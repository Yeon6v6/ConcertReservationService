...
public class SeatReservedEventListener {
    ...
    public void handleBeforeCommit(ConcertSeatReservedEvent event) {
    ...
    public void handleAfterCommit(ConcertSeatReservedEvent event) {
        log.info("[ReservationEventListener] 트랜잭션 커밋 후 Kafka 발행 시작: reservationId={}", event.getReservationId());
        kafkaProducer.sendMessage(event.getTopic(), event.getKey(), event.getPayload());
    }
}
